  ...
  socket.on("market_data", (data) => {
    ...
    if (data.stocks) {
      ...
      window.dispatchEvent(
        new CustomEvent("stocksUpdated", { detail: data.stocks })
      );
    }
    if (data.holdings) {
      console.log("Updating portfolio:", data.holdings);
      window.dispatchEvent(
        new CustomEvent("portfolioUpdated", { detail: data.holdings })
      );
    }
    if (data.leaderboard) {
      console.log("Updating leaderboard:", data.leaderboard);
      window.dispatchEvent(
        new CustomEvent("leaderboardUpdated", { detail: data.leaderboard })
      );
    }
  });
